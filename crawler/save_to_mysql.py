...
class save_to_database(object):
    ...
    def save_refusedURLS(self, data_list):
        conn = pymysql.connect(host=hostD, port=portD, user=userD, passwd=passwdD, db=dbD,
                               charset=charsetD)  # 基本的本机MYSQL配置
        cursor = conn.cursor()
        sql = "insert into refused_enter_urls (id,first_page,second_page,detail_model,error_info) VALUES (%s,%s,%s,%s,%s)"
        cursor.execute(sql, data_list)
        conn.commit()
        print('【无法打开页面信息】插入成功！')
        conn.close()

    def delete_from_error(self, id_f):
        conn = pymysql.connect(host=hostD, port=portD, user=userD, passwd=passwdD, db=dbD,
                               charset=charsetD)  # 基本的本机MYSQL配置
        cursor = conn.cursor()
        sql = "delete from refused_enter_urls where id = "+ str(id_f)
        cursor.execute(sql)
        conn.commit()
        print('【错误信息】删除成功！')
        conn.close()
